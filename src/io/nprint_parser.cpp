...
void NprintParser::perform() {
	...
	t = pcap_dump_open(pd, config.outfile.c_str());
    /*  header */
    getline(instream, line);
    while (getline(instream, line)) {
	    bits = clean_line(line);
	    packet_info = parse_packet(bits);
	    if (std::get<0>(packet_info) != nullptr) {
...
NprintParser::NprintParser(const Config &config, const FileWriter &fileWriter)
		: IOParser(config, fileWriter) {
